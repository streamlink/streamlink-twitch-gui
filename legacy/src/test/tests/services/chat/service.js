...
module( "services/chat", function( hooks ) {
	...
	/** @typedef {TestContext} TestContextChatService */
	/** @this TestContextChatService */
	...
	test( "Opens chat", async function( assert ) {
		...
		assert.ok(
			logDebugStub.calledWith( "Preparing to launch chat" ),
			"Logs preparing to launch"
		);
		assert.ok(
			logErrorStub.calledWithExactly( error ),
			"Calls logError with error"
		);
	});
});
