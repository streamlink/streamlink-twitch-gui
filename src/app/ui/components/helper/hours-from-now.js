...
export const helper = FromNowHelper.extend({
	...
	_compute( [ time = 0 ] ) {
		...
		if ( diff < minute / 2 ) {
			return this.intl.t( "helpers.hours-from-now.now" ).toString();
			...
			return this.intl.t( "helpers.hours-from-now.minutes", {
				minutes: minutes < 10
					? `0${minutes.toFixed()}`
					: minutes.toFixed()
			}).toString();
			...
			if ( remaining > minute ) {
				return this.intl.t( "helpers.hours-from-now.hours.extended", {
					hours: Math.floor( diff / hour ).toFixed(),
					minutes: Math.floor( remaining / minute ).toFixed()
				}).toString();

			} else {
				return this.intl.t( "helpers.hours-from-now.hours.simple", {
					hours: Math.floor( diff / hour ).toFixed()
				}).toString();
			}
			...
			if ( remaining > hour ) {
				return this.intl.t( "helpers.hours-from-now.days.extended", {
					days: Math.floor( diff / day ).toFixed(),
					hours: Math.floor( remaining / hour ).toFixed()
				}).toString();

			} else {
				return this.intl.t( "helpers.hours-from-now.days.simple", {
					days: Math.floor( diff / day ).toFixed()
				}).toString();
			}
		}
