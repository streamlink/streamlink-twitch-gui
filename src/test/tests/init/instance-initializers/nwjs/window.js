...
module( "init/instance-initializers/nwjs/window", function( hooks ) {
	setupTest( hooks, {
		resolver: buildResolver()
	});

	/** @typedef {TestContext} TestContextInstanceInitializersNwjsWindow */
	/** @this TestContextInstanceInitializersNwjsWindow */
	...
	hooks.afterEach(function() {
		// tick twice for "time-window-event-debounce", to make EmberQUnit's isSettled() happy
	...
	test( "Non-existing window record", async function( assert ) {
		...
		const windowRecord = store.peekRecord( "window", 1 );
		assert.propEqual(
